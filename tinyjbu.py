...
class TinyVitJBU(Module):
    ...
    def __init__(self,  use_norm):
        ...
        backbone = TinyViT(img_size=self.input_size,
        ...
        if use_norm:
            self.backbone = torch.nn.Sequential(backbone, ChannelNorm(self.embed_dims[-1]))
        else:
            self.backbone = backbone
        self.upsampler = get_upsampler("jbu_stack", self.embed_dims[-1])

    def forward(self, image):
        features = self.backbone(image)  # torch.Size([1, 784, 128])  torch.Size([1, 196, 160]) torch.Size([1, 49, 320]) torch.Size([1, 49, 320])
